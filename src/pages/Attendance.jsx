...
// Formats a total-seconds duration into a short "1h 25m" style string,
// same convention used for the worked-time badge.
const formatBreakSeconds = (totalSecs) => {
...
// NAYA: "05:34 pm" jaisi 12-hour clock_in string ko minutes-since-midnight
// me convert karta hai, taaki actual chronological order se compare ho
// sake — plain string comparison "11:48 am" vs "05:34 pm" pe galat
// result deta tha (kyunki '1' > '0' lexicographically), jabki 05:34 pm
// asal me baad ka time hai.
const parseClockInToMinutes = (timeStr) => {
  if (!timeStr) return -1;
  ...
  let [, hourRaw, minuteRaw, meridiemRaw] = match;
...
const Attendance = () => {
  ...
  const [attendance, setAttendance] = useState([]);
  const [compOffHistoryMap, setCompOffHistoryMap] = useState({});
  // NAYA: employeeId-date -> formatted break duration string (e.g. "1h 25m")
  ...
  // NAYA: har unique employee ke liye break summary fetch karke
  // `${employeeId}-${date}` key se ek flat map bana rahe hain,
  // taaki table render karte waqt O(1) lookup ho.
  const fetchBreakSummary = async (records) => {
    const employeeIds = [
      ...new Set(records.map((record) => record.employee_id).filter(Boolean)),
    ];
    ...
    try {
      const responses = await Promise.all(
        employeeIds.map(async (employeeId) => {
          const res = await api.get(
            `/break/attendance-summary/${employeeId}`,
            {
              params: {
                startDate: filters.startDate,
                endDate: filters.endDate,
          ...
          return {
            employeeId: String(employeeId),
            // { 'YYYY-MM-DD': { total_seconds, formatted } }
      ...
      const nextMap = {};
      responses.forEach(({ employeeId, summary }) => {
        Object.entries(summary).forEach(([date, value]) => {
          nextMap[`${employeeId}-${date}`] = value;
        });
      });

      setBreakSummaryMap(nextMap);
    } catch (error) {
      console.error("Error fetching break summary:", error);
      setBreakSummaryMap({});
    }
  };
  ...
  const fetchAttendance = async () => {
    try {
      ...
      setAttendance(records);
      await fetchCompOffHistory(records);
      await fetchBreakSummary(records);
    } catch (err) {
      console.error("Error fetching attendance:", err);
      setCompOffHistoryMap({});
      setBreakSummaryMap({});
  ...
  // NAYA: jab ek hi employee ke same date pe multiple attendance
  // sessions hote hain (jaise 3 alag clock-in/out cycles), us group
  // me sabse latest clock_in wale record ko yahan pehchana ja raha
  // hai — taaki table me use "Latest" badge se highlight kar sakein.
  const latestEntryRecords = useMemo(() => {
    const groups = new Map();

    attendance.forEach((record) => {
      const workDate = formatDateOnly(record.date);
      if (!record.employee_id || !workDate) return;

      const key = `${record.employee_id}-${workDate}`;
      const existing = groups.get(key);

      if (
        !existing ||
        parseClockInToMinutes(record.clock_in) >
          parseClockInToMinutes(existing.clock_in)
      ) {
  ...
  // NAYA: given ek attendance record, uske liye break summary entry lookup karo
  const getBreakEntry = (record) => {
    const workDate = formatDateOnly(record.date);
    if (!record.employee_id || !workDate) {
      return null;
    }

    return breakSummaryMap[`${record.employee_id}-${workDate}`] || null;
  };
  ...
  // NAYA: breakSummaryMap ki har key ek unique employee+date hai (chahe
  // us din ke kitne bhi attendance sessions ho), isliye seedha values
  // sum karne se sahi total break time milega, duplicate counting nahi hogi.
  const totalBreakSeconds = useMemo(
    () =>
      Object.values(breakSummaryMap).reduce(
        (sum, entry) => sum + (Number(entry?.total_seconds) || 0),
        0,
      ),
    [breakSummaryMap],
  );

  const totalBreakLabel = useMemo(() => {
    const secs = Math.max(0, Math.floor(totalBreakSeconds));
    const hrs = Math.floor(secs / 3600);
    const mins = Math.floor((secs % 3600) / 60);
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{totalBreakLabel}</p>
              <p className="text-xs text-gray-500 mt-0.5">Total Break Time</p>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center gap-4">
            <div className="p-2.5 rounded-lg bg-amber-50">
              ...
              <Table>
                ...
                <TableBody>
                  {filteredAndSorted.map((record, index) => {
                    const isMissing = record.worked_time === "Missing Clock Out";
                    const compOffEntry = getCompOffEntry(record);
                    const breakEntry = getBreakEntry(record);
                    const breakLabel = breakEntry
                      ? formatBreakSeconds(breakEntry.total_seconds)
                      : null;
                    const isLatestEntry = latestEntryRecords.has(record);
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="px-2 py-2">{getStatusCell(record.worked_time)}</TableCell>
                        <TableCell className="px-2 py-2 whitespace-nowrap">
                          {isLatestEntry && breakLabel ? (
                            <span className="inline-flex items-center gap-1 text-[11px] font-medium text-orange-700 bg-orange-50 border border-orange-200 px-1.5 py-0.5 rounded-md">
